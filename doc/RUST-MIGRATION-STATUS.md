...
**Trạng thái:** Đã loại bỏ thư mục `server/` (Node). Backend chạy hoàn toàn trên Rust (`server-rs/`). `pnpm dev` và `paperclipai run` khởi động Rust server. Cấu hình backend (pool DB, scheduler, HTTP, runner, CORS, security headers, metrics, config file) theo env — xem `doc/RUST-BACKEND-OPTIMIZATION-PLAN.md` (Phase 1–4 đã áp dụng).
