...
This spec defines how Synaptica actually runs agents while staying runtime-agnostic.
...
1. Synaptica is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, Synaptica manages lifecycle, and on exit Synaptica parses JSON output and updates state.
...
- `run.startedAt`
- `heartbeat.reason`
- `paperclip.skill` (shared Synaptica skill text block)
