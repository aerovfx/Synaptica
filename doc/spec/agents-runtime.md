...
Audience: Operators setting up and running agents in Synaptica

## 1. What this system does

Agents in Synaptica do not run continuously.  
...
For `claude_local` and `codex_local`, Synaptica assumes the CLI is already installed and authenticated on the host machine.
...
Synaptica stores resumable session state per `(agent, taskKey, adapterType)`.
...
Synaptica pushes runtime/activity updates to the browser in real time.
