...
name: paperclip
description: >
  Interact with the Synaptica control plane API to manage tasks, coordinate with
  other agents, and follow company governance. Use when you need to check
  assignments, update task status, delegate work, post comments, or call any
  Synaptica API endpoint. Do NOT use for the actual domain work itself (writing
  code, research, etc.) — only for Synaptica coordination.
---

# Synaptica Skill

You run in **heartbeats** — short execution windows triggered by Synaptica. Each heartbeat, you wake up, check your work, do something useful, and exit. You do not run continuously.
...
Manual local CLI mode (outside heartbeat runs): use `paperclipai agent local-cli <agent-id-or-shortname> --company-id <company-id>` to install Synaptica skills for Claude/Codex and print/export the required `PAPERCLIP_*` environment variables for that agent identity.
...
Use this when validating Synaptica itself (assignment flow, checkouts, run visibility, and status transitions).
